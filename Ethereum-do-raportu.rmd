---
title: "Ethereum - Podsumowanie wyników strategii"
output:
  pdf_document:
...
df_BH <- Ethereum %>%
  filter(Date %in% common_dates) %>%
  select(Date, returns, capital)

df_M12 <- Ethereum %>%
  filter(Date %in% common_dates) %>%
  select(Date, momentum_return, momentum_capital)
...
Strategia Buy & Hold dla Ethereum wykazała ekstremalnie wysoką roczną stopę zwrotu na poziomie 266,99% (w ujęciu geometrycznym), co doprowadziło do kapitału końcowego przekraczającego 3 miliony dolarów z początkowego tysiąca. Współczynnik Sharpe na poziomie 1,68 potwierdza skuteczną relację zwrotu do ryzyka, jednak maksymalne obsunięcie kapitału sięgające -94,05% wskazuje na katastrofalną ekspozycję na spadki rynkowe. Strategia działa zgodnie z założeniami długoterminowego inwestowania w aktywa o wysokim potencjale wzrostu, ale jej implementacja wymaga od inwestora niezwykłej odporności psychicznej i zdolności do przetrwania okresów niemal całkowitej utraty kapitału.\


**Momentum 12-1**\

Strategia Momentum 12-1 generuje bardzo wysoką roczną stopę zwrotu na poziomie 134% przy wciąż znaczącej, choć nieco niższej zmienności (97,12% rocznie). Główną zaletą jest częściowe ograniczanie ekspozycji w okresach spadkowych, co przełożyło się na nieco płytsze maksymalne obsunięcie kapitału (-87,71%). Jednak strategia nie wykazuje istotnej statystycznie przewagi nad pasywną inwestycją (p-value=0,1144). Współczynnik Sharpe na poziomie 1,51 wskazuje na dobrą efektywność w relacji zysk-ryzyko. Strategia pełni funkcję narzędzia kontroli ryzyka w porównaniu do BH, oferując zbliżone potencjalne zwroty przy nieco złagodzonych wahaniach, choć wciąż na poziomie ekstremalnym.\


**Mean Reversion (Lookback 2 dni)**\

Strategia Mean Reversion wykazała zupełnie inną dynamikę, osiągając umiarkowaną roczną stopę zwrotu na poziomie 16,82% przy wciąż wysokiej zmienności (101,71%). Współczynnik Sharpe wynoszący 0,66 wskazuje na słabą relację zysku do ryzyka, a maksymalne obsunięcie kapitału na poziomie -76,66% można uznać za głębokie. Strategia działała niezgodnie z założeniami na silnie trendującym rynku kryptowalut, co sugeruje, że mechanizm powrotu do średniej nie sprawdza się w warunkach długotrwałych, wyraźnych trendów charakterystycznych dla Ethereum.\


**Volatility Targeting (Cel = 10%, Okno = 60 dni)**\

Strategia Volatility Targeting wykazała doskonałe wyniki w kontroli ryzyka, osiągając roczną stopę zwrotę na poziomie 24,65% przy radykalnie niskiej zmienności (10,95% rocznie). Współczynnik Sharpe wynoszący 1,99 wskazuje na bardzo dobrą relację zysku do ryzyka, a maksymalne obsunięcie kapitału na poziomie zaledwie -8,73% można uznać za płytkie. Test t z p-value=0,0097 dla porównania z BH potwierdza statystyczną istotność przewagi strategii w zakresie profilu ryzyka. Strategia działa zgodnie z założeniami dynamicznego dostosowywania ekspozycji do bieżącej zmienności rynku, skutecznie chroniąc kapitał w okresach wysokiej niepewności przy zachowaniu dodatnich zwrotów.\


**Porównanie**\

Analiza porównawcza wszystkich strategii na danych out-of-sample pokazuje fundamentalny dylemat inwestycyjny na rynku Ethereum. Pod względem końcowej wartości kapitału, strategia Buy & Hold osiągnęła zdecydowanie najlepszy wynik, potwierdzając ogromny potencjał wzrostowy aktywa w długim okresie. Strategia Momentum, choć osiągnęła wysokie zwroty, nie zdołała znacząco poprawić profilu ryzyka w sposób statystycznie istotny. Strategia Mean Reversion okazała się nieadekwatna do warunków rynkowych. Strategia Volatility Targeting zaskakująco zdominowała pod względem kontroli ryzyka, osiągając najlepszy współczynnik Sharpe i radykalnie ograniczając maksymalne straty, co czyni ją najbezpieczniejszym wyborem dla inwestorów wrażliwych na wahania kapitału.